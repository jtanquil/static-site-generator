...
class TestSplitNodesDelimiter(unittest.TestCase):
  def setUp(self):
    ...
    self.test_cases["no_inline_elements"] = TextNode("no inline elements", text_type_text)
    self.test_results["no_inline_elements"] = [TextNode("no inline elements", text_type_text)]
    
    self.test_cases["entire_element_inline"] = TextNode("**entire element is inline**", text_type_bold)
    self.test_results["entire_element_inline"] = [TextNode("entire element is inline", text_type_bold)]

    self.test_cases["beginning_inline"] = TextNode("`beginning` is inline", text_type_code)
    self.test_results["beginning_inline"] = [TextNode("beginning", text_type_code), TextNode(" is inline", text_type_text)]

    self.test_cases["end_inline"] = TextNode("end is *inline*", text_type_italic)
    self.test_results["end_line"] = [TextNode("end is ", text_type_text), TextNode("inline", text_type_italic)]

    self.test_cases["multiple_inline"] = TextNode("**multiple** elements **are** in**line**", text_type_bold)
    ...
    self.multiple_nodes = [self.test_cases["no_inline_elements"], self.test_cases["multiple_inline"]]
    self.multiple_nodes_test_results = self.test_results["no_inline_elements"].extend(self.test_results["multiple_inline"])

    self.exceptions["unclosed_inline_beginning"] = TextNode("`unclosed element", text_type_code)
    self.exceptions["unclosed_inline_end"] = TextNode("another unclosed element*", text_type_italic)
    self.exceptions["unclosed_inline_mid"] = TextNode("middle ** unclosed element", text_type_bold)

  def test_split_nodes_delimiter(self):
    for case in self.test_cases:
      text_type = self.test_cases[case].text_type
      result = split_nodes_delimiter([self.test_cases[case]], text_type_delimiters[text_type], text_type)
    ...
    for exception in self.exceptions:
      self.assertRaises(ValueError, split_nodes_delimiter, self.exceptions[exception])
